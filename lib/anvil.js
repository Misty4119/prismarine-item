function loader (registry, Item) {
  ...
  function combine (itemOne, itemTwo, creative, renamedName) {
    ...
    if (itemOne) {
      ...
      if (finalItem.componentMap) {
        finalItem.components = itemOne.components.map(component => JSON.parse(JSON.stringify(component)))
        finalItem.removedComponents = [...itemOne.removedComponents]
        finalItem.componentMap = new Map(finalItem.components.map(component => [component.type, component]))
      }
  ...
  function combineEnchants (itemOne, itemTwo, creative) {
    ...
    const finalEnchs = itemOne.enchants.map(ench => ({ ...ench }))
    ...
    for (const ench of itemTwoEnch) {
      ...
      const itemCategories = registry.itemsByName[itemOne.name].enchantCategories ?? []
      const canApply = itemCategories.includes(category) || (category === 'durability' && itemCategories.includes('breakable'))
      if (!(itemOne.name === 'enchanted_book' && rightIsBook) && !canApply && !creative) continue
