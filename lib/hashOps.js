...
function crc32c (buffer) {
  let crc = -1
  for (const byte of buffer) crc = CRC32C_TABLE[(crc ^ byte) & 0xff] ^ (crc >>> 8)
  return (crc ^ -1) >>> 0
}
...
const types = {
  hash: [unreadable, function (value, buffer, offset, args, context) {
    const body = Buffer.alloc(this.sizeOf(value, args.body, context))
    this.write(value, body, 0, args.body, context)
    const digest = crc32c(body)
    const result = args.type.startsWith('i') ? digest | 0 : digest
    return this.write(result, buffer, offset, args.type, context)
  }, function (value, args, context) {
    return this.sizeOf(0, args.type, context)
  }],
